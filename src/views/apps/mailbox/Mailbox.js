...
const MailboxApp = () => {
  ...
  const { flights, showSettings, loading, selectedItems, viewingMail } = useSelector((state) => state.mailbox);

  const { themeValues } = useSelector((state) => state.settings);
  const lgBreakpoint = parseInt(themeValues.lg.replace('px', ''), 10);

  const { width } = useWindowSize();
  const [listingItems, setListingItems] = useState(flights);
